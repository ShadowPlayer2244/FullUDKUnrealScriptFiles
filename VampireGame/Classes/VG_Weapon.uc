...
//=============================================================================
class VG_Weapon extends Weapon;
...
{
	local VG_PlayerPawn VG_PlayerPawn;
	...
	{
		// Cast the new owner into a VG_PlayerPawn as we need the weapon socket name
		// If the cast succeeds, we check that the new owner's mesh has a socket by that name
		VG_PlayerPawn = VG_PlayerPawn(NewOwner);
		if (VG_PlayerPawn != None && NewOwner.Mesh.GetSocketByName(VG_PlayerPawn.WeaponSocketName) != None)
		{
			...
			Mesh.SetLightEnvironment(VG_PlayerPawn.LightEnvironment);
			// Attach the weapon mesh to the new owner's skeletal meshes socket
			NewOwner.Mesh.AttachComponentToSocket(Mesh, VG_PlayerPawn.WeaponSocketName);
		}
	}
}
